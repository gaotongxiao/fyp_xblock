...
EJABBERD_CHAT_SERVER_IP_ADDRESS = 'ejab.dyfyp.club'
...
class Ejabberd(object):
    ...
    def chat_url(self):
        ...
        return str(
            'http://%s:80/http-bind/' % EJABBERD_CHAT_SERVER_IP_ADDRESS)
    ...
    def admin_url(self):
        ...
        """
        return str('http://%s:8081/rest/' % EJABBERD_CHAT_SERVER_IP_ADDRESS)
    ...
    def get_user_rooms(self, user_id):
        user, host = user_id.split('@')
        response = self._make_post_request(
            'get_user_rooms %s %s' % (user, host))
